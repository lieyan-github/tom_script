...
批量对比路径字符串(_in路径列表1, _in路径列表2){
    ...
    ; 对比路径列表
    if(_in路径列表1.Length() = _in路径列表2.Length()){

        Loop % _in路径列表1.Length(){

            if(_in路径列表1[A_Index] = _in路径列表2[A_Index]){

                _ret["相同的列表"].push(_in路径列表1[A_Index])

            }Else{

                _tmpList := []
                _tmpList.push(_in路径列表1[A_Index], _in路径列表2[A_Index])
                _ret["不同的列表"].push(_tmpList)
        ...
        show_msg(Format("路径列表1[{1}] 与 路径列表2[{2}] 长度不同, 批量对比操作取消, 返回空字符串!"
                        , _in路径列表1.Length()
                        , _in路径列表2.Length()))
        _ret := ""
    }

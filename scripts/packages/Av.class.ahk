...
; 2.从_数据表文件"已看过的_精品av女优.txt"文件中, 读取所有非空行到 _数据列表
    ...
    if(_result>0){
        ...
        if(_in操作类型 = "update"){
            ...
            _log .= format("`n更改索引{1} `n`n新内容: {2}", _result, _分析后的csv行)
        }
    }
    else{
        ; 新项目, 进行追加
        ...
        if(_in操作类型 = "add" || _in操作类型 = "update"){
            ...
            _log .= format("`push追加索引{1} `n`n新内容: {2}", _数据列表.MaxIndex(), _分析后的csv行)
        }
    }
    ; 保存到文件
    CsvFile.writeListToCsv(_数据列表, _数据表文件)
    show_msg(format("{1}: {2} `n`n编辑内容已保存 `n`n{3}", _数据表类型, _输入数据行, _log))
    ; end
    return _result
}

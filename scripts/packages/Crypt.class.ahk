...
class Crypt
{
    class Encrypt
    {
        ...
        StrDecryptToFile(EncryptedHash,pFileOut,password,CryptAlg = 1, HashAlg = 1) 
        {
            if !EncryptedHash
                return ""
            if !len := CryptFuncs.b64Decode( EncryptedHash, encr_Buf )
                return ""
        ...
        FileEncryptToStr(pFileIn,password,CryptAlg = 1, HashAlg = 1) 
        {
            ...
            {
                CryptFuncs.Free(tembBuf)
                return ""
            }
            f.Close()
            FileDelete,% temp_file
            return CryptFuncs.b64Encode( tembBuf, fLen )
        }
        ...
        StrEncrypt(string,password,CryptAlg = 1, HashAlg = 1)
        {
            ...
            if this._Encrypt(str_buf,len, password, 1,0,0,CryptAlg,HashAlg)
                return CryptFuncs.b64Encode( str_buf, len )
            else
                return ""
        }
...
FINITA_LA_COMEDIA:
    ...
    class Hash
    {
        ...
        _CalcHash(ByRef bBuffer,BufferLen,pFile,HashAlg = 1,pwd = "",hmac_alg = 1)
        {
            ...
            hashval := CryptFuncs.b2a_hex( pbHash, HashLen )

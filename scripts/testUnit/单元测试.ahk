class testContent{
    ...
    test(){
        ...
        Clipboard := ""
        assert("Av.rename av女优-001"
                , Av.rename(avstr)
                , "#av女优# ★★★ 佐山愛 1990-00-00 tags(标签) " . Sys.date() . " - 演員 - 影片 - AVMOO")

        ; 收藏夹重命名av女优, 从剪贴板获取生日和罩杯
        avstr := "佐山愛 - 演員 - 影片 - AVMOO"
        Clipboard := "1998年10月30日 g罩杯"
        assert("Av.rename av女优-002"
                , Av.rename(avstr) 
                , "#av女优# ★★★ 佐山愛 1998-10-30 tags(长相好看 乳g) " . Sys.date() . " - 演員 - 影片 - AVMOO")

        ; Av.rename 收藏夹重命名av作品
        ; ----------------------------------------------------------
        avstr := "RBD-930 強制受胎闇市場 佐山愛_真木今日子 - AVMOO"
        assert("Av.rename av作品-001"
                , Av.rename(avstr)
                , "#av作品# ★★★ RBD-930 強制受胎闇市場 佐山愛_真木今日子 tags(标签) " . Sys.date() . " - AVMOO")

        ; 对收藏夹内容格式化
        avstr := "#av作品# ★★★ RBD-930 強制受胎闇市場 佐山愛_真木今日子 tags(美颜_乳g_诱惑) 2019年10月30日 - AVMOO"
        assert("Av.rename av作品-002"
                , Av.rename(avstr) 
                , "#av作品# ★★★ RBD-930 強制受胎闇市場 佐山愛_真木今日子 tags(长相好看｜乳g｜诱惑) 2019-10-30 - AVMOO")

        ; 模板_tags_format 识别下划线间隔的多个标签
        ; ----------------------------------------------------------
        avstr := "tags(美颜_丰乳h_美臀)"
        assert("模板_tags_format-001", 模板_tags_format(avstr), "tags(长相好看｜丰乳h｜美臀)")

        avstr := "#精品女优#(美颜_丰乳h_美臀)"
        assert("模板_tags_format-002", 模板_tags_format(avstr), "tags(长相好看｜丰乳h｜美臀)")

        avstr := "(美颜_丰乳h_美臀)"
        assert("模板_tags_format-003", 模板_tags_format(avstr), "tags(长相好看｜丰乳h｜美臀)")

        ; AvInfoAnalysis
        ; ----------------------------------------------------------
        ; 000 av女优 识别
        avstr := "#av女优# ★★★ 佐山愛 1989-01-08 tags(美颜 丰乳h 美臀)"
        assert("AvInfoAnalysis av女优 识别", list_join(AvInfoAnalysis.parse(avstr).提取tags(avstr)), "美颜 丰乳h 美臀")
        ...
        ; 000 标准格式化分析
        avstr := "#av女优# ★★★ 佐山愛 1989-01-08 tags(美颜 丰乳h 美臀)"
        assert("AvInfo.提取tags-000", list_join(AvInfo.提取tags(avstr)), "美颜 丰乳h 美臀")

        ; 001 特殊情况分析 tags下划线
        avstr := "#av女优# ★★★ 佐山愛 1989-01-08 tags(美颜_丰乳h_美臀)"
        assert("AvInfo.提取tags-001", list_join(AvInfo.提取tags(avstr)), "美颜 丰乳h 美臀")

        ; 002 特殊情况分析 tags只有括号且下划线间隔
        avstr := "#av女优# ★★★ 佐山愛 1989-01-08 (美颜_丰乳h_美臀)"
        assert("AvInfo.提取tags-002", list_join(AvInfo.提取tags(avstr)), "美颜 丰乳h 美臀")
        ...
        ; path
        ; ----------------------------------------------------------
        assert("Path-001", true, Path.has_ext("C:\Windows\hh.exe"))
        assert("Path-002", true, Path.has_ext("C:\Windows\hh.1exe"))
        assert("Path-003", true, Path.is_dir("C:\Windows\System32"))

        ; path test
        _path := Path.parse("C:\Windows\notepad.exe")

        assert("Path-004", _path.file, "notepad.exe")
        assert("Path-005", _path.dir, "C:\Windows")
        assert("Path-006", _path.ext, "exe")
        assert("Path-007", _path.file_no_ext, "notepad")
        assert("Path-008", _path.drive, "C:")

        ; path test 2 对不含扩展名 split自动检测处理
        _path := Path.parse("C:\Windows\notepad")

        assert("Path-009", _path.file, "notepad")
        assert("Path-010", _path.ext, "")
        assert("Path-011", _path.file_no_ext, "notepad")
    }
}
